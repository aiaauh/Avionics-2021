...
//Modified by Raj and Jo 06/02/2022 to clean up code and modify SD Card headers
//Modified by Kurt 06/14/2022 to convert from SPI to I2C for simplicity of wiring the perf board
//Boards:
//   Teensy 4.1
...
#define SEALEVELPRESSURE_HPA (1013.25)  //Note: We will need to adjust this to match pressure at launch site. //Updated 6/6/22 New Mexico Weather
Adafruit_BMP3XX bmp;
...
/*String getHeader() {
  ...
  String header = "Time(ms)\tBMP_Temp(C)\tBMP_Temp(F)\tBMP_Pre(hPa)\tBMP_Pre(PSI)\tBMP_Alt(m)\tBMP_Alt(ft)\tLIS_X_Acc(m/s2)\tLIS_Y_Acc(m/s2)\tLIS_Z_Acc(m/s2)\t";
  header += "LSM_Temp(C)\tLSM_Temp(F)\tLSM_X_Acc(m/s2)\tLSM_Y_Acc(m/s2)\tLSM_Z_Acc(m/s2)\tLSM_X_Gyro(rad/s)\tLSM_Y_Gyro(rad/s)\tLSM_Z_Gyro(rad/s)";
...
//  ** Important ** Only uncomment the while loop delay 10 in the above code for troubleshooting!
//                  If this line is left uncommented in the rocket, it will wait for Serial Monitor
//                  and will never run the main loop or record data until a PC is connected.
  ...
  dso32.setAccelDataRate(LSM6DS_RATE_26_HZ);    // SHUTDOWN, 12_5_HZ, 26_HZ, 52_HZ, 104_HZ, 208_HZ, 416_HZ, 833_HZ, 1_66K_HZ, 3_33K_HZ, 6_66K_HZ
  Serial.print("Accelerometer data rate set to: 26 Hz\n");
  dso32.setGyroDataRate(LSM6DS_RATE_26_HZ);    // SHUTDOWN, 12_5_HZ, 26_HZ, 52_HZ, 104_HZ, 208_HZ, 416_HZ, 833_HZ, 1_66K_HZ, 3_33K_HZ, 6_66K_HZ
  Serial.print("Gyro data rate set to: 26 Hz\n");
...
void loop() {  // This is the main loop that runs ifinitly
  ...
  String rawData = millis();       //Create string with time since Teensy powered ON (ms)
  ...
  if (dataFile) {
    ...
    Serial.println(rawData);    // Print the string in Serial Monitor
  }
  else {
    Serial.println("error opening ROCKET.txt for loop");  // if the file isn't open, pop up an error
  }
}
